...
public class FieldEffectManager : ManagerBase<FieldEffectManager>
{
    ...
    public void CreateFieldEffect(EffectData data, Vector3 pos)
    {
        var go = ResourcesManager.GetInstance().CreateEffectObjectInstance(data.code, this.gameObject.transform, pos);
        ...
        if (go != null)
        {
            ...
            if (effectCtrl == null)
            {
                Logger.SError("could not find effect controller in effect " + data.code);
                return;
            }
    ...
    public void CreateFieldEffect(ulong effectCode, Vector3 pos)
    {
